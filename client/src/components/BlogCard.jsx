...
const BlogCard = ({ title, content, author, date, category }) => {
  return (
    <div className="bg-white rounded-2xl shadow-xl p-0 mb-8 max-w-xl w-full mx-auto border border-gray-200 hover:shadow-2xl transition-all duration-300 group overflow-hidden">
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-2xl font-extrabold text-indigo-700 transition-colors duration-200 truncate flex-1">{title}</h3>
          {category && (
            <span className="bg-gradient-to-r from-blue-500 to-purple-600 text-white text-xs font-semibold px-3 py-1 rounded-full shadow-md ml-3">
              {category}
            </span>
          )}
        </div>
