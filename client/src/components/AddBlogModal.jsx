...
const AddBlogModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const { backendUrl,setBlogCount } = useContext(AppContent);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post(backendUrl + "/api/blogs/add", {
        title,
        content,
      });
      if (data.success) {
        toast.success(data.message);
        setBlogCount(count=>count+1);
        setTitle("");
        setContent("");
        onClose();
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-blue-200/60 via-purple-200/60 to-pink-200/60 backdrop-blur-[4px]">
      ...
      >
        <div
          className="bg-white/80 rounded-3xl shadow-2xl border-4 border-transparent bg-clip-padding p-10 sm:p-12 relative overflow-hidden"
        ...
        >
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            <h2 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-purple-600 to-pink-500 mb-1 tracking-tight">
              Add New Blog
            </h2>
